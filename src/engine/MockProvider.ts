...
const builtIns: Record<string, MockScenario> = {
  happy_path: {
    steps: [
      { type: 'message', text: 'analyzed task' },
      { type: 'final', text: 'completed' },
    ],
  },
  multi_step: {
    steps: [
      { type: 'message', text: 'planned' },
      { type: 'message', text: 'checked tools' },
      { type: 'final', text: 'completed after multiple steps' },
    ],
  },
  blocked: {
    steps: [
      { type: 'tool_call', tool: 'execute_bash', args: { command: 'rm -rf /tmp/huascar-mock' } },
      { type: 'final', text: 'blocked action recorded' },
    ],
  },
  timeout: {
    steps: [
      { type: 'message', text: 'waiting', delay_ms: 10 },
      { type: 'error', text: 'mock timeout' },
    ],
  },
...
function loadMockScenario(
  ...
  if (config.paths.mockScenarios) {
    ...
    const custom =
      asScenario(parsed) ??
      (parsed && typeof parsed === 'object' ? asScenario((parsed as Record<string, unknown>)[name]) : null);
...
export async function runMockScenario(options: {
  ...
  for (const step of scenario.steps) {
    ...
    if (step.type === 'final')
      return [...lines, `final: ${template(step.text, options.task, scenarioName)}`].join('\n');
    if (step.type === 'error')
      throw new Error(`[MOCK:${scenarioName}] ${template(step.text, options.task, scenarioName)}`);
  }
