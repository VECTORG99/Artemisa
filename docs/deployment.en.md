...
- **`ARTEMISA_API_KEYS`**, **`BYPASS_SECRET`** and **`METRICS_SECRET`** should never be committed. Use `.env` (gitignored) for local development, or Render's secret environment variables for production.
- The backend automatically redacts `BYPASS_SECRET` from logs.
...
No hay restricciones de recursos establecidas en `docker/docker-compose.yml`. Para producción, considere agregar:
...
> El Creator sin estado usa significativamente menos memoria que el runtime anterior. 256M es un límite inicial razonable; ajústelo según el uso observado.

### Verificación Post-Despliegue

Después de `make docker-up`, verifique que todos los servicios estén en funcionamiento:
...
Para despliegues públicos, los servicios exponen HTTP sin cifrado. Agregue un proxy inverso (Caddy, Nginx, Traefik) delante para la terminación TLS. Caddy es el más simple: un solo binario, HTTPS automático mediante Let's Encrypt:
...
El Creador es sin estado y determinista: cada solicitud es una función pura de su cuerpo. Puedes ejecutar múltiples réplicas detrás de un equilibrador de carga sin afinidad de sesión, almacenamiento compartido o enrutamiento fijo. El único estado por instancia son las métricas en proceso y los búferes de depuración, que no se comparten.
