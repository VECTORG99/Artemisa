...
| `HUASCAR_API_KEYS` | **Yes** (when `AUTH_REQUIRED=true`) | —       | Comma-separated valid API keys. Sent via `Authorization: Bearer` or `X-API-Key`.      |
...
3. `HUASCAR_API_KEYS`, `BYPASS_SECRET` and `METRICS_SECRET` are auto-generated by Render.
4. Deploy.
...
- **`HUASCAR_API_KEYS`**, **`BYPASS_SECRET`** and **`METRICS_SECRET`** should never be committed. Use `.env` (gitignored) for local dev, or Render's secret env vars for production.
