...
- Keep separate install domains and duplicate shared types manually in both backend and frontend: rejected; caused drift risk between Creator catalog/workflow contracts on each side.
...
- Move `agent-creator/` and `frontend/` dependencies fully into the root `package.json`: rejected; keeps app-specific tooling (Vite vs Next) isolated, workspaces only shares what's declared.

## Consequences

- `packages/types` changes propagate to backend and frontend without manual copying.
- Single root lockfile (`package-lock.json`) now covers backend, frontend, and agent-creator; frontend dependency churn does rewrite the root lockfile.
