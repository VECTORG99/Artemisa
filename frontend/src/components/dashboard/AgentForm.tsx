...
export function AgentForm({ role, roles, task, loading, onRoleChange, onTaskChange, onDeploy }: AgentFormProps) {
  return (
    <form
      className="bg-zinc-900 border border-zinc-800 rounded-lg p-6 flex flex-col gap-6"
      onSubmit={(event) => {
    ...
    >
      <h2 id="agent-form-title" className="text-xl font-semibold text-zinc-100">
      ...
      <div className="flex flex-col gap-2 flex-1">
        <label htmlFor="task" className="text-sm font-medium text-zinc-300">
          Tarea a Ejecutar
        </label>
        <textarea
          id="task"
