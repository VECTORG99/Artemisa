  ...
  return (
    <>
      <section
        ...
        className="bg-black border border-zinc-800 rounded-lg p-4 flex-1 font-mono text-sm overflow-hidden flex flex-col min-h-[300px]"
      >
        ...
        <div
          ...
          aria-busy={loading}
          aria-label="Registro de ejecución del agente"
        >
          {logs.map((log, i) => (
            <div key={i} className={logClass(log)}>
              <span className="text-zinc-600 mr-2" aria-hidden="true">
                {'>'}
              </span>
              {log}
            </div>
