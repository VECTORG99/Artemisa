...
export default function ReviewStep() {
  ...
  const display = (question, value) => {
    ...
    return values
      .map((item) => itemById.get(item) || optionMap.get(item) || String(item).replace('custom:', 'Personalizado: '))
      .join(', ');
  };
        ...
        <header className="mb-8 flex flex-wrap items-end justify-between gap-4">
          <div>
            ...
            <p className="mt-3 max-w-2xl text-zinc-400">
              Revisa decisiones y recomendaciones. Generar crea un preview descargable; no ejecuta herramientas ni
                ...
                return (
                  ...
                  >
                    <span className="min-w-0">
                      ...
                      <span className="mt-1 block break-words font-medium text-zinc-100">
          ...
          <div className="space-y-6">
                  ...
                  {evaluation.recommendations.map((item) => (
                    <article key={item.id} className={`rounded-xl border p-4 ${severityStyle[item.severity]}`}>
                      ...
                      <details className="mt-3 text-xs text-zinc-400">
                        ...
                        <p className="mt-2">
                          <strong>Beneficios:</strong> {item.benefits.join('; ')}
                        </p>
                        <p className="mt-1">
                          <strong>Trade-offs:</strong> {item.tradeoffs.join('; ')}
                        </p>
                        <p className="mt-1">
                          <strong>Alternativas:</strong> {item.alternatives.join('; ')}
                        </p>
                      </details>
                    </article>
                  ))}
            ...
            {evaluation.warnings.length > 0 && (
              <section className="rounded-2xl border border-amber-900/60 bg-amber-950/20 p-5">
                ...
                <ul className="mt-3 space-y-2 text-sm text-zinc-300">
                  {evaluation.warnings.map((message) => (
            ...
            {loading && (
              <p role="status" aria-live="polite" className="sr-only">
                Generando el preview y sus artefactos.
              </p>
            )}
            {error && (
