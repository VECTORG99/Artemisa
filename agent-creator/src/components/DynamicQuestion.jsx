  ...
  return (
    ...
    >
      ...
      <div className="flex items-start gap-3">
        <span
          className={`mt-0.5 flex h-5 w-5 shrink-0 items-center justify-center border ${multiple ? 'rounded' : 'rounded-full'} ${selected ? 'border-emerald-400 bg-emerald-500 text-zinc-950' : 'border-zinc-600'}`}
        >
          {selected ? '✓' : ''}
        </span>
        <span>
...
export default function DynamicQuestion() {
  ...
  const catalogOptions = useMemo(() => {
    ...
    return catalog.items.filter(
      (item) =>
        ...
        (!query ||
          [item.label, item.description, item.id, ...item.tags].some((text) => text.toLowerCase().includes(query))),
    );
  ...
  const renderInput = () => {
    ...
    if (question.type === 'boolean') {
      return (
        <fieldset className="grid gap-3 sm:grid-cols-2">
          ...
          <Choice
            name={question.id}
            option={{ label: 'Sí', description: 'Activar esta capacidad en el blueprint.' }}
            selected={value === true}
            onChange={() => updateAnswer(question.id, true)}
          />
          <Choice
            name={question.id}
            option={{ label: 'No', description: 'Omitirla y documentar la decisión.' }}
        ...
        <fieldset className="grid gap-3 md:grid-cols-2">
          ...
          {options.map((item) => (
            <Choice
              ...
              option={item}
              multiple={multiple}
              selected={multiple ? selected.includes(item.id) : value === item.id}
    ...
    const categories = question.catalogCategories
  ...
  return (
    <div key={question.id} className="animate-fade-in">
      ...
      <h2 id={`${question.id}-prompt`} className="text-2xl font-bold text-zinc-50 sm:text-3xl">
        {question.prompt}
      </h2>
      <p className="mb-6 mt-3 text-zinc-400">{question.description}</p>
      {renderInput()}
      {question.maxSelections && multiple && (
        <p className="mt-3 text-xs text-zinc-500">
          Seleccionadas: {selected.length} / {question.maxSelections}
        </p>
      )}
      {currentIssue && (
        <p role="alert" aria-live="polite" className="mt-3 text-sm text-red-400">
