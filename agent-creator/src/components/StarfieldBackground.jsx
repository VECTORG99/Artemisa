...
export default function StarfieldBackground() {
  ...
  useEffect(() => {
    ...
    const meteors = Array.from({ length: 2 }).map(() => ({
      x: 0,
      y: 0,
      length: 0,
      speed: 0,
    ...
    const spawnMeteor = (meteor) => {
      ...
      meteor.angle = Math.PI / 4 + (Math.random() * 0.2 - 0.1);
      meteor.alpha = 0;
      meteor.isRgb = Math.random() > 0.8;
    };
    ...
    const render = () => {
      ...
      meteors.forEach((meteor) => {
        ...
        if (meteor.isRgb) {
          ...
          gradient.addColorStop(0, `rgba(161, 161, 170, ${meteor.alpha})`);
